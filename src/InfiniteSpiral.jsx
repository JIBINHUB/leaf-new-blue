/**
 * InfiniteSpiral — a helix of images turning past the viewer.
 *
 * Component supplied by the project owner. Kept as provided apart from this
 * header and one change, marked inline: the animation frame loop now stops
 * when the spiral is off screen or the tab is in the background. As supplied
 * it ran from mount for the life of the page, which in this single-page app
 * means a loop repainting ten cards behind every other route a visitor moves
 * to — and on a phone, forever.
 *
 * Its stylesheet is authored locally in InfiniteSpiral.css, which the supplied
 * snippet does not include.
 *
 * Worth noting why this one is safe where the ring it replaces was not: it
 * computes x and z itself and emits translate3d/rotateZ/scale only. There is
 * no transform-style: preserve-3d anywhere in it, which is the thing mobile
 * Safari and Chrome fail to paint on this site.
 */
...
const InfiniteSpiral = ({
  ...
  useEffect(() => {
    ...
    /* Added: the loop only runs while the spiral is on screen and the tab is in
       the foreground. Upstream started it on mount and never stopped it — in a
       single-page app that is ten cards being repainted every frame behind
       every other route, for as long as the tab is open. One frame is still
       painted on stopping, so the spiral holds its position rather than
       snapping when it comes back. */
    const start = () => {
      if (running) return;
      running = true;
    ...
    // One frame now, so the cards are placed before the first scroll.
    frameId = requestAnimationFrame(render);
