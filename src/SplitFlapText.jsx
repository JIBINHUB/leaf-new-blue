/**
 * SplitFlapText — a departure-board that flips between phrases.
 *
 * Component supplied by the project owner; kept as provided apart from this
 * header. Its accompanying stylesheet is authored locally in SplitFlapText.css,
 * which the supplied snippet does not include.
 */
...
/* Written as character codes rather than literal escapes: both of these are
   control/invisible characters, and pasting them as literals is fragile — they
   get silently normalised in transit, which turns the separator into an empty
   string and makes split() break the phrase list into single characters. */
const SEPARATOR = String.fromCharCode(31); // unit separator, never in a phrase
const NBSP = String.fromCharCode(160);     // keeps blank tiles from collapsing
...
const SplitFlapText = ({
  ...
  const phrasesKey = typeof text === 'string' ? text : sourceWords.map(word => String(word ?? '')).join(SEPARATOR);
